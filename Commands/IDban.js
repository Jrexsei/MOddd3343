...
exports.run = async(client, message, args) => {
    ...
    message.channel.send(embed.setDescription(`<@!${user.id}> - (\`${user.id}\`) Adlı kullanıcı \`${sebep}\` sebebiyle yasaklandı.`)).then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
