...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author}, Eksik arguman kullandınız, \`.ban @etiket/ID reklam\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout:10000 }))

    let banLimit = await kdb.get(`banlimit.${message.author.id}`)
    if (banLimit >= ayar.banLimit) {
        return message.channel.send(embed.setDescription(`${message.author}, Ban limit sınırına ulaştın limitin sıfırlanmasını bekle!`))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    }
