...
exports.run = async(client, message, args) => {
    ...
    message.channel.send(embed.setDescription(`${victim} - ${message.author} Adlı kullanıcı ses kanalına gelmek istiyor kabul ediyor musun ?`)).then(async(msg) => {
        ...
        onay.on("collect", async() => {
            ...
            message.channel.send(embed.setDescription(`${victim} Adlı kullanıcı odaya gelmenizi onayladı.`)).then(m => m.delete({ timeout: 10000 }))
            msg.delete({ timeout: 10000 })
            message.delete()
        })
        red.on("collect", async() => {
            await msg.reactions.removeAll()
            message.channel.send(embed.setDescription(`${victim} Adlı kullanıcı odaya gitmenizi reddetti.`)).then(m => m.delete({ timeout: 10000 }))
            msg.delete({ timeout: 10000 })
            message.delete()
        })
    })
