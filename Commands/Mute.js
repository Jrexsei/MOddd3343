...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author}, Eksik arguman kullandınız, \`.mute @etiket/ID 1m Küfür\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    if (!args[1]) return message.channel.send(embed.setDescription(`${message.author}, Eksik arguman kullandınız, \`.mute @etiket/ID 1m Küfür\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    let sure = args[1]
        .replace("s", " Saniye")
        .replace("m", " Dakika")
        .replace("h", " Saat")
...
${user} Adlı kullanıcı ${message.author} tarafından \`${reason}\` sebebiyle  \`${sure}\` boyunca metin kanallarında susturuldu. Ceza ID: \`#${cezaID}\``))//.then(x => x.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    ...
    setTimeout(async() => {
        ...
        if (log) log.send(embed.setDescription(`${user} Adlı kullanıcının metin kanallarındaki susturulması bitti.`))
