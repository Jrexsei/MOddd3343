...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author}, Eksik arguman kullandınız, \`.jail @etiket/ID 1m Küfür\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    ...
    if (durum) return message.channel.send(embed.setDescription(`${user} Adlı kullanıcının aktif bir **JAİL** cezası bulunmakta.`))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
...
${user} Adlı kullanıcı ${message.author} tarafından \`${reason}\` sebebiyle  cezalandırıldı. Ceza ID: \`#${cezaID}\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
