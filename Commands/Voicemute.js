...
exports.run = async(client, message, args) => {
    ...
    if (!user) return message.channel.send(embed.setDescription(`${message.author}, Eksik arguman kullandınız, \`.vmute @etiket/ID 1m Küfür\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    if (!user.voice.channel) return message.react(ayar.no)
    if (!args[1]) return message.channel.send(embed.setDescription(`${message.author}, Eksik arguman kullandınız, \`.vmute @etiket/ID 1m Küfür\``))//.then(m => m.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
    let sure = args[1]
        .replace("s", " Saniye")
        .replace("m", " Dakika")
        .replace("h", " Saat")
    ...
    if (durum) return message.channel.send(embed.setDescription(`${user} Adlı kullanıcının aktif bir **VOİCE-MUTE** cezası bulunmakta.`))//.then(x => x.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
...
${user} Adlı kullanıcı ${message.author} tarafından \`${reason}\` sebebiyle  \`${sure}\` boyunca ses kanallarında susturuldu. Ceza ID: \`#${cezaID}\``))//.then(x => x.delete({ timeout: 10000 }) && message.delete({ timeout: 10000 }))
