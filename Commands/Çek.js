...
exports.run = async(client, message, args) => {
    ...
    message.channel.send(embed.setDescription(`${victim} - ${message.author} Adlı kullanıcı ses kanalına çekmek istiyor kabul ediyor musun ?`)).then(async(msg) => {
        ...
        const onayemoji = (reaction, user) => reaction.emoji.id === "850426418000297995" && user.id === victim.id;
        const redemoji = (reaction, user) => reaction.emoji.id === "850426436934041610" && user.id === victim.id;
        ...
        onay.on("collect", async() => {
            ...
            message.channel.send(embed.setDescription(`${victim} Adlı kullanıcı odaya çekmenizi onayladı.`)).then(m => m.delete({ timeout: 10000 }))
            msg.delete({ timeout: 10000 })
            message.delete()
        })
        red.on("collect", async() => {
            await msg.reactions.removeAll()
            message.channel.send(embed.setDescription(`${victim} Adlı kullanıcı odaya çekmenizi reddetti.`)).then(m => m.delete({ timeout: 10000 }))
            msg.delete({ timeout: 10000 })
            message.delete()
        })
    })
